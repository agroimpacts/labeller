function init(gridJson, kmlName, assignmentId, tryNum, resultsAccepted, refJson, workJson, xyzAttributes, snapTolerance, mmJson) {
    ...
    var map = new ol.Map({
        ...
        layers: [
            ...
            new ol.layer.Group({
                title: 'Base Layer',
                layers: [bingLayer, esriLayer, mapboxLayer]
            })
        ],
    ...
    if (!workerFeedback) {
        var fieldsLayer = new ol.layer.Vector({
            title: "Mapped Fields",
            zIndex: 101,
            // load machine_maps geojson into fieldslayer
            source: new ol.source.Vector({
                features: new ol.Collection().extend(new ol.format.GeoJSON().readFeatures(mmJson))
            }),
