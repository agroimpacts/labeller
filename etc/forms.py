...
def password_validator(form, field):
    """ Password must be at least 6 characters long with one lowercase letter, one uppercase letter and one digit. """
...
class InviteForm(FlaskForm):
    email = StringField(_('Email'), validators=[
        validators.DataRequired(_('Email is required')),
        validators.Email(_('Invalid Email'))])
    # Role choices are computed at runtime in views.invite()
    role = SelectField(label='Role (select one)', choices=[])
    next = HiddenField()
