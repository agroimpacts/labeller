...
def invite():
    ...
    if hasattr(db_adapter.UserInvitationClass, 'role'):
        ...
        # Compute the actual invitee role choices allowed for the logged-in user.
        choices = Role.query.filter(Role.invited_by.in_((current_user.get_roles()))).all()
        invite_form.role.choices = [(choice.name, choice.label) for choice in choices]
        if len(invite_form.role.choices) == 0:
    ...
    if request.method=='POST' and invite_form.validate():
        ...
        # Store role
        if hasattr(db_adapter.UserInvitationClass, 'role'):
            user_invite.role = invite_form.role.data
            db_adapter.commit()
...
def _do_login_user(user, safe_next, remember_me=False):
    ...
    if not _call_or_get(user.is_active):
        flash(_('Your account is not enabled.'), 'error')
        return redirect(url_for('user.login'))
