## Author: Lei Song
## To update the database after clearing the database
## if it is initial, update master_grid and scenes_data table
## if it is the regular production, 
## update the configuration proportion values in DB to generate initial F sites
## This simply makes sure all the initial Fs are valid
...
if (params$labeller$initial == 1 | params$labeller$initial == 2) {
  ...
  train_static <- s3read_using(
    read.csv,
    object = file.path(
      params$learner$prefix, params$learner$incoming_names_static
