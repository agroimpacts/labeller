...
selected_fields <- lapply(hostname, function(x) { # x <- hostname[1]
  # print(x)
  host <- paste0(x, ".", host_root)
...
names(selected_fields) <- hostname
...
for(i in 1:length(dat)) {
  ...
  for(j in 1:nrow(dat[[i]]$grid)) {  # i <- 1; j <- 1
    ...
    qaqc_maps <- NULL  # dummy for qaqc_maps
    cat(chunk_text[1], file = outfile, append = TRUE)
    cat_strings <- c(
      nl(glue("qaqc_maps <- NULL")),  # dummy for qaqc_maps
      nl(glue("d <- dat[[{i}]]")),
    ...
    cat(chunk_text[1], file = outfile, append = TRUE)
    cat(nl(glue(
      "display_imagery(gpoly, user_maps, qaqc_maps, os_url, gs_url, d$cats)")
    ), file = outfile, append = TRUE)
    cat(chunk_text[2], file = outfile, append = TRUE)
